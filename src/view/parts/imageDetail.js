...
export class ImageDetail extends BaseView {
  ...
  render(store, actionData) {
    const toNativeSizeButton = div("", ["toNativeSizeButton"], {
      on: {
        click: this.toNativeSize()
      }
    }, "1:1");
    const toWindowSizeButton = div("", ["toWindowSizeButton"], {
      on: {
        click: this.toWindowSize()
      }
    }, "W");
    const toDoubleWindowSizeButton = div("", ["toDoubleWindowSizeButton"], {
      on: {
        click: this.toDoubleWindowSize()
      }
    }, "Wx2");
    const toHelfWindowSizeButton = div("", ["toHelfWindowSizeButton"], {
      on: {
