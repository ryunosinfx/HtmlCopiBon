import { PrefActionCreator } from '../action/settingActionCreator'
...
export class PrefViewReducer extends BaseReducer {
	...
	async update(data) {
		...
		const saved = await this.prefm.save(pk, data.name, data.pageNum, data.startPage, data.pageDirection, data.outputProfile, data.listing);
