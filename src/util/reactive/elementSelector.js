// import { patch } from './base/preLoader.js';
import { ESVnode } from '../../../lib/ESVnode.js';
import { ObjectUtil } from '../objectUtil.js';
const patch = ESVnode.init();
export class ElementSelector {
	constructor() {
		console.log('ElementSelector constructor');
		this.selectorMap = new Map();
	}
	getElementById(vnode, id) {
		const result = this.getElements(vnode, '#' + id);
	...
	isValidNode(vnode, name, parentNode, index) {
		...
		if (vnode.children) {
			for (const indexA in vnode.children) {
				let child = vnode.children[indexA];
				if (!child) {
					continue;
				}
				this.isValidNode(child, name + '_c_', vnode.elm ? vnode.elm.parentNode : null, indexA);
			}
		}
	}
	log(msg, ...a) {
	...
	prePatch(vnode, selector, newNode) {
		...
		this.log('prepatch01a　nodes：' + nodes, nodes, parentMap.size + '/' + cloneNode.children);
		for (const node of nodes) {
			this.log('prepatch01a');
			const newOne = ObjectUtil.deepVnodeClone(newNode);
			this.log('prepatch01b');

			for (const [key, parentNode] of parentMap) {
				this.log(key + '/' + node);
				if (key === node) {
					...
					for (const index in children) {
						...
						if (target === node) {
							...
							isShouldPatch = true;
							this.log(
								`★patch01c AS REPLACE! children.length:${children.length}/index:${index}/target.sel:${target.sel}/newOne.sel:${newOne.sel}/text:${newOne.text}`
							);
							break;
						} else {
							this.log(
								`-patch01d ! children.length:${children.length}/index:${index}/target.sel:${target.sel}`
							);
						}
					}
					break;
		...
		return isShouldPatch ? cloneNode : vnode;
	}
