...
export class Store {
	...
	static getTemp(newKey = mainKey, action) {
		const store = subHolder.has(action) ? subHolder.get(action) : null;
		return store;
	}
	...
	static setStore(store, newKey = mainKey, actionClass) {
		const clonedStore = store ? ObjectUtil.simpleDeepClone(store) : {};
		const storeAtCurrent = mainHolder.has(newKey) ? mainHolder.get(newKey) : null;
		for (const index in storeAtCurrent) {
