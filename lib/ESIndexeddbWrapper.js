...
export class idbw {
	...
	async getAccessor(tn = idbw.crrntDBN, cT, kpn = idbw.kpnA) {
		return await IA.gi(tn, kpn, this.dbn, cT);
	}
}
class IA {
	#i = N;
	...
	static SBBA = N;
	constructor(tn, kpn = idbw.cnst.kpn, isAI = 0, cdbN = IA.dbName, cT, l = console) {
		const z = this;
		if (!IA.hM.has(cdbN)) IA.hM.set(cdbN, new IH(cdbN, cT));
		z.#i = IA.hM.get(cdbN);
		console.log('z.#i:' + z.#i);
		z.kpn = kpn;
		z.tn = tn;
	...
	static async gi(tn, kpN, cdbn = IA.dbName, cT, isAI = 0, isC = 1) {
		const k = Js([cdbn, tn]);
		if (IA.aM.has(k)) return IA.aM.get(k);
		if (!IA.SBBA) IA.SBBA = await IA.mki(idbw.CDBNa, idbw.kpn, 0, idbw.sysDBN, cT, 0);
		return await IA.mki(tn, kpN, isAI, cdbn, cT, isC, k);
	}
	static async mki(tn, kpn, isAI = 0, cdbn = IA.dbName, cT, isC = 1, k) {
		const i = new IA(tn, kpn, isAI, cdbn, cT);
	...
	getAsMap(ks) {
		return isArr(ks) ? this.#i.sByKeys(this.tn, ks, this.isC) : N;
	}
	getRecord(k) {
...
class IC {
	...
	cDB(v) {
		...
		ct(z.t);
		z.t = st(() => io(`BeCloseDB! closeTimeout:${z.cT}/dbn:${z.dbn}`, IC.beCDB(z)), z.cT);
		return v;
	}
	static beCDB(z, v) {
		z.db.close();
